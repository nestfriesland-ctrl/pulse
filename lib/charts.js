...
(function () {
  ...
  async function initBtcChartWithMarkers(containerId, markers) {
    ...
    async function load() {
      ...
      series.setData(candles);
      if (markers) {
        const tryLine = (price, color, title) => {
          ...
          } catch (e) { /* older builds don't expose createPriceLine */ }
        };
        tryLine(markers.entry, c.ink,  'entry');
        tryLine(markers.sl,    c.bear, 'SL');
        tryLine(markers.tp1,   c.bull, 'TP1');
        tryLine(markers.tp2,   c.bull, 'TP2');
      }
      chart.timeScale().fitContent();
      lastCandle = candles[candles.length - 1];
      caption.textContent = `BTC · 4h · ${candles.length} candles · close ${fmtUsd(lastCandle.close)} on ${fmtDate(lastCandle.time)} · Kraken`;
